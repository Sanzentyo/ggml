...
static void concat_cuda(const T *  x,
    ...
    if (dim == 0) {
        const ggml_cuda_kernel_launch_params launch_params =
            ggml_cuda_kernel_launch_params(num_blocks, CUDA_CONCAT_BLOCK_SIZE, 0, stream);
    ...
    if (dim == 1) {
        const ggml_cuda_kernel_launch_params launch_params =
            ggml_cuda_kernel_launch_params(num_blocks, CUDA_CONCAT_BLOCK_SIZE, 0, stream);
        ggml_cuda_kernel_launch(concat_cont<1, T>, launch_params, x, y, dst, ne00, ne01, ne02, ne0, ne1, ne2);
        return;
    }
    const ggml_cuda_kernel_launch_params launch_params =
        ggml_cuda_kernel_launch_params(num_blocks, CUDA_CONCAT_BLOCK_SIZE, 0, stream);
    ggml_cuda_kernel_launch(concat_cont<2, T>, launch_params, x, y, dst, ne00, ne01, ne02, ne0, ne1, ne2);
}
...
template <typename T>
static void ggml_cuda_op_concat_t(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (ggml_is_contiguous(src0) && ggml_is_contiguous(src1)) {
        ...
        auto launch_kernel = [&](auto dim) {
            concat_non_cont<dim, T><<<grid_dim, CUDA_CONCAT_BLOCK_SIZE, 0, stream>>>(
...
void ggml_cuda_op_concat(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (dst->type == GGML_TYPE_F16) {
        ggml_cuda_op_concat_t<half>(ctx, dst);
    } else if (dst->type == GGML_TYPE_BF16) {
        ggml_cuda_op_concat_t<nv_bfloat16>(ctx, dst);
    } else if (dst->type == GGML_TYPE_F32) {
        ggml_cuda_op_concat_t<float>(ctx, dst);
    } else {
