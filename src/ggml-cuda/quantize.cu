...
template <mmq_q8_1_ds_layout ds_layout, bool has_ids>
...
static __global__ void quantize_mmq_q8_1_ds4_warp_cols(
        const float * __restrict__ x, void * __restrict__ vy,
        const int64_t ne00, const int64_t s01, const int64_t s02, const int64_t s03,
    ...
    const int64_t base = i3 * s03 + i2 * s02 + i1 * s01 + i0;
    const float4 xi = make_float4(i0 + 0 < ne00 ? x[base + 0] : 0.0f,
                                  i0 + 1 < ne00 ? x[base + 1] : 0.0f,
                                  i0 + 2 < ne00 ? x[base + 2] : 0.0f,
                                  i0 + 3 < ne00 ? x[base + 3] : 0.0f);
...
template <int cols_per_block, mmq_activation activation_static>
static __global__ void quantize_mmq_q8_1_ds4_warp_cols_inplace_activation(
        float * __restrict__ x, void * __restrict__ vy,
        const int64_t ne00, const int64_t s01, const int64_t s02, const int64_t s03,
        const int64_t ne1, const int64_t ne2) {
    constexpr int vals_per_mmq_block = 4 * QK8_1;
    ...
    const int64_t base = i3 * s03 + i2 * s02 + i1 * s01 + i0;
    const float4 raw = make_float4(i0 + 0 < ne00 ? x[base + 0] : 0.0f,
                                   i0 + 1 < ne00 ? x[base + 1] : 0.0f,
                                   i0 + 2 < ne00 ? x[base + 2] : 0.0f,
                                   i0 + 3 < ne00 ? x[base + 3] : 0.0f);
    const float4 xi = make_float4(mmq_apply_activation_static<activation_static>(raw.x, activation_static),
                                  mmq_apply_activation_static<activation_static>(raw.y, activation_static),
                                  mmq_apply_activation_static<activation_static>(raw.z, activation_static),
                                  mmq_apply_activation_static<activation_static>(raw.w, activation_static));
...
template <int cols_per_block, mmq_activation activation_static>
static void launch_quantize_mmq_q8_1_ds4_warp_cols_inplace_activation(
        float * x, void * vy, const int64_t ne00, const int64_t s01, const int64_t s02, const int64_t s03,
        const int64_t ne0, const int64_t ne1, const int64_t ne2, const int64_t ne3, cudaStream_t stream) {
    const int64_t qblocks = ne0 / (4 * QK8_1);
    const dim3 num_blocks((ne1 + cols_per_block - 1) / cols_per_block, qblocks, ne2 * ne3);
    const dim3 block_size(WARP_SIZE, cols_per_block, 1);
    quantize_mmq_q8_1_ds4_warp_cols_inplace_activation<cols_per_block, activation_static>
        <<<num_blocks, block_size, 0, stream>>>(x, vy, ne00, s01, s02, s03, ne1, ne2);
}

template <mmq_activation activation_static>
static void launch_quantize_mmq_q8_1_ds4_inplace_activation(
        float * x, void * vy, const int cols_per_block, const int64_t ne00, const int64_t s01, const int64_t s02,
        const int64_t s03, const int64_t ne0, const int64_t ne1, const int64_t ne2, const int64_t ne3, cudaStream_t stream) {
    if (cols_per_block == 16) {
        launch_quantize_mmq_q8_1_ds4_warp_cols_inplace_activation<16, activation_static>(
            x, vy, ne00, s01, s02, s03, ne0, ne1, ne2, ne3, stream);
    } else if (cols_per_block == 8) {
        launch_quantize_mmq_q8_1_ds4_warp_cols_inplace_activation<8, activation_static>(
            x, vy, ne00, s01, s02, s03, ne0, ne1, ne2, ne3, stream);
    } else {
        launch_quantize_mmq_q8_1_ds4_warp_cols_inplace_activation<4, activation_static>(
            x, vy, ne00, s01, s02, s03, ne0, ne1, ne2, ne3, stream);
    }
}
...
void quantize_mmq_q8_1_cuda(
    ...
    if (ids == nullptr && type_src0 == GGML_TYPE_Q4_1 && ne1 >= 16384 &&
        getenv("GGML_CUDA_DISABLE_MMQ_Q8_1_WARP_COL_QUANT") == nullptr) {
        const int64_t qblocks = ne0 / (4 * QK8_1);
        const int cols_per_block_env = [] {
            const char * env = getenv("GGML_CUDA_MMQ_Q8_1_WARP_COLS");
            return env == nullptr ? 8 : atoi(env);
        }();
        if (cols_per_block_env == 4) {
            const dim3 num_blocks((ne1 + 3) / 4, qblocks, ne2 * ne3);
            const dim3 block_size(WARP_SIZE, 4, 1);
            quantize_mmq_q8_1_ds4_warp_cols<4><<<num_blocks, block_size, 0, stream>>>(x, vy, ne00, s01, s02, s03, ne1, ne2);
        } else if (cols_per_block_env == 16) {
            const dim3 num_blocks((ne1 + 15) / 16, qblocks, ne2 * ne3);
            const dim3 block_size(WARP_SIZE, 16, 1);
            quantize_mmq_q8_1_ds4_warp_cols<16><<<num_blocks, block_size, 0, stream>>>(x, vy, ne00, s01, s02, s03, ne1, ne2);
        } else {
            const dim3 num_blocks((ne1 + 7) / 8, qblocks, ne2 * ne3);
            const dim3 block_size(WARP_SIZE, 8, 1);
            quantize_mmq_q8_1_ds4_warp_cols<8><<<num_blocks, block_size, 0, stream>>>(x, vy, ne00, s01, s02, s03, ne1, ne2);
        }
        return;
    }
    ...
    switch (mmq_get_q8_1_ds_layout(type_src0)) {
        case MMQ_Q8_1_DS_LAYOUT_D4:
            if (ids) {
                ...
                quantize_mmq_q8_1<MMQ_Q8_1_DS_LAYOUT_D4, false>
                    <<<num_blocks, block_size, 0, stream>>>(x, nullptr, vy, ne00, s01, s02, s03, ne0, ne1, ne2);
            }
            break;
        case MMQ_Q8_1_DS_LAYOUT_DS4:
            if (ids) {
                quantize_mmq_q8_1<MMQ_Q8_1_DS_LAYOUT_DS4, true>
                    <<<num_blocks, block_size, 0, stream>>>(x, ids, vy, ne00, s01, s02, s03, ne0, ne1, ne2);
            } else {
        ...
        case MMQ_Q8_1_DS_LAYOUT_D2S6:
            if (ids) {
                quantize_mmq_q8_1<MMQ_Q8_1_DS_LAYOUT_D2S6, true>
                    <<<num_blocks, block_size, 0, stream>>>(x, ids, vy, ne00, s01, s02, s03, ne0, ne1, ne2);
            } else {
...
void quantize_mmq_q8_1_inplace_activation_cuda(
        float * x, void * vy, const ggml_type type_src0, const mmq_activation activation,
        const int64_t ne00, const int64_t s01, const int64_t s02, const int64_t s03,
        const int64_t ne0, const int64_t ne1, const int64_t ne2, const int64_t ne3, cudaStream_t stream) {
    GGML_ASSERT(ne00 % 4 == 0);
    GGML_ASSERT(ne0 % (4*QK8_1) == 0);
    GGML_ASSERT(type_src0 == GGML_TYPE_Q4_1);
    GGML_ASSERT(mmq_get_q8_1_ds_layout(type_src0) == MMQ_Q8_1_DS_LAYOUT_DS4);
    GGML_ASSERT(activation != MMQ_ACT_NONE);

    const int cols_per_block_env = [] {
        const char * env = getenv("GGML_CUDA_MMQ_Q8_1_ACT_WARP_COLS");
        return env == nullptr ? 4 : atoi(env);
    }();
    switch (activation) {
        case MMQ_ACT_GELU:
            launch_quantize_mmq_q8_1_ds4_inplace_activation<MMQ_ACT_GELU>(
                x, vy, cols_per_block_env, ne00, s01, s02, s03, ne0, ne1, ne2, ne3, stream);
            break;
        case MMQ_ACT_GELU_ERF:
            launch_quantize_mmq_q8_1_ds4_inplace_activation<MMQ_ACT_GELU_ERF>(
                x, vy, cols_per_block_env, ne00, s01, s02, s03, ne0, ne1, ne2, ne3, stream);
            break;
        case MMQ_ACT_GELU_QUICK:
            launch_quantize_mmq_q8_1_ds4_inplace_activation<MMQ_ACT_GELU_QUICK>(
                x, vy, cols_per_block_env, ne00, s01, s02, s03, ne0, ne1, ne2, ne3, stream);
            break;
