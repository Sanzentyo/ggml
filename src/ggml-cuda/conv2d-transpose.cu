...
static __global__ void conv2d_transpose_k2_s2_kernel(const float * __restrict__ input,
    ...
    if (in_x >= in_w || in_y >= in_h) {
        output[(out_w * out_h * c_out) * n_idx + (out_w * out_h) * c_idx + (out_w) * out_y_idx +
               out_x_idx] = 0.0f;
        return;
    }
...
void ggml_cuda_conv_2d_transpose_p0(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (kernel_w == 2 && kernel_h == 2 && stride == 2 && ggml_cuda_conv_transpose_k2s2_enabled()) {
        if (kernel->type == GGML_TYPE_F16) {
            conv2d_transpose_k2_s2_kernel<half><<<blocks, CUDA_CONV2D_TRANSPOSE_BLOCK_SIZE, 0, st>>>(
                input_data, (const half *) kernel_data, output_data, input_w, input_h, output_w, output_h,
                channels_in, channels_out, batches);
        } else {
            conv2d_transpose_k2_s2_kernel<float><<<blocks, CUDA_CONV2D_TRANSPOSE_BLOCK_SIZE, 0, st>>>(
