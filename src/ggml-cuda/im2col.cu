...
static void im2col_3d_cuda_bf16(const float * src, nv_bfloat16 * dst,
    int64_t N, int64_t IC, int64_t ID, int64_t IH, int64_t IW, int64_t OC,
    int64_t KD, int64_t KH, int64_t KW, int64_t OD, int64_t OH, int64_t OW,
    int64_t stride_q, int64_t stride_z, int64_t stride_y, int64_t stride_x,
    int s0, int s1, int s2, int p0, int p1, int p2, int d0, int d1, int d2, cudaStream_t stream) {

    im2col_3d_cuda<nv_bfloat16>(src, dst, N, IC, ID, IH, IW, OC, KD, KH, KW, OD, OH, OW,
                                stride_q, stride_z, stride_y, stride_x,
                                s0, s1, s2, p0, p1, p2, d0, d1, d2, stream);
}
...
void ggml_cuda_op_im2col_3d(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if(dst->type == GGML_TYPE_F16) {
        ...
        im2col_3d_cuda_bf16(src1_d, (nv_bfloat16 *) dst_d, N, IC, ID, IH, IW, OC, KD, KH, KW, OD, OH, OW,
                            stride_q, stride_z, stride_y, stride_x,
                            s0, s1, s2, p0, p1, p2, d0, d1, d2, stream);
    } else {
