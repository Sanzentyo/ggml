...
#define WARP_SIZE 32
#define CUDART_HMAX \
    11070  // CUDA 11.7, min. ver. for which __hmax and __hmax2 are known to work (may be higher
           // than needed)
...
// While BW spans CC 1000, 1100 & 1200, we are integrating Tensor Core instructions available to
// 1200 family, see
...
#define GGML_CUDA_CC_GCN4 \
    (GGML_CUDA_CC_OFFSET_AMD + 0x803)  // Tonga, Fiji, Polaris, minimum for fast fp16
#define GGML_CUDA_CC_VEGA \
    (GGML_CUDA_CC_OFFSET_AMD + 0x900)  // Vega56/64, minimum for fp16 dual issue
#define GGML_CUDA_CC_VEGA20 (GGML_CUDA_CC_OFFSET_AMD + 0x906)  // MI50/Radeon VII, minimum for dp4a
#define GGML_CUDA_CC_CDNA1 \
    (GGML_CUDA_CC_OFFSET_AMD + 0x908)  // MI100, minimum for MFMA, acc registers
#define GGML_CUDA_CC_CDNA2 \
    (GGML_CUDA_CC_OFFSET_AMD + 0x90a)  // MI210 (gfx90a), minimum acc register renaming
...
#define GGML_CUDA_CC_IS_MTHREADS(cc) \
    (cc >= GGML_CUDA_CC_OFFSET_MTHREADS && cc < GGML_CUDA_CC_OFFSET_AMD)
...
template <class... Archs>
constexpr int ggml_cuda_highest_compiled_arch_impl(const int arch,
                                                   const int cur,
                                                   const int first,
                                                   Archs... rest) {
    ...
    do {                                                                         \
        ...
        if (!shared_memory_limit_raised[id]) {                                   \
            CUDA_CHECK(cudaFuncSetAttribute(                                     \
                kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, nbytes));   \
...
#if (!defined(GGML_USE_HIP) && !defined(GGML_CUDA_NO_VMM)) || \
    (defined(GGML_USE_HIP) && !defined(GGML_HIP_NO_VMM))
#define GGML_USE_VMM
#endif  // (!defined(GGML_USE_HIP) && !defined(GGML_CUDA_NO_VMM)) || (defined(GGML_USE_HIP) &&
        // !defined(GGML_HIP_NO_VMM))
...
// The Volta instructions are in principle available on Turing or newer but they are effectively
// unusable:
...
#define AMPERE_MMA_AVAILABLE
    ...
    __CUDA_ARCH__ < GGML_CUDA_CC_RUBIN
...
static bool fp16_mma_hardware_available(const int cc) {
    return (GGML_CUDA_CC_IS_NVIDIA(cc) && cc >= GGML_CUDA_CC_VOLTA) || GGML_CUDA_CC_IS_CDNA(cc) ||
...
static bool bf16_mma_hardware_available(const int cc) {
    return (GGML_CUDA_CC_IS_NVIDIA(cc) && cc >= GGML_CUDA_CC_AMPERE) || GGML_CUDA_CC_IS_CDNA(cc) ||
           cc >= GGML_CUDA_CC_RDNA3 || (GGML_CUDA_CC_IS_MTHREADS(cc) && cc >= GGML_CUDA_CC_PH1);
}
...
[[noreturn]]
static __device__ void no_device_code(const char* file_name,
                                      const int line,
...
#else
    printf(
        "%s:%d: ERROR: CUDA kernel %s has no device code compatible with CUDA arch %d. "
...
#define NO_DEVICE_CODE \
    no_device_code(__FILE__, __LINE__, __FUNCTION__, __CUDA_ARCH__, STRINGIZE(__CUDA_ARCH_LIST__))
#else
...
template <block_reduce_method reduce_method_t,
          const unsigned int block_size_template = 0,
          typename T>
...
#endif  // (defined(CUDART_VERSION) && CUDART_VERSION < CUDART_HMASK) || defined(GGML_USE_HIP) ||
        // (defined(MUSART_VERSION) && MUSART_VERSION < MUSART_HMASK)
...
#if defined(GGML_USE_HIP) && \
    (defined(RDNA2) || defined(RDNA3) || defined(RDNA4) || defined(__gfx906__) || defined(CDNA))
#define V_DOT2_F32_F16_AVAILABLE
#endif  // defined(GGML_USE_HIP) && (defined(RDNA2) || defined(RDNA3) || defined(RDNA4) ||
        // defined(__gfx906__) || defined(CDNA))
...
// Aligned memory transfers of 8/16 bytes can be faster than 2 transfers with 4 bytes, especially on
// AMD. Important: do not use this function if dst and src both point at registers.
...
//     make the compiler emit the right instructions. If dst and src point at different address
//     spaces then they are guaranteed to not be aliased.
template <int nbytes, int alignment = 0>
static __device__ __forceinline__ void ggml_cuda_memcpy_1(void* __restrict__ dst,
    ...
    static_assert(nbytes <= ggml_cuda_get_max_cpy_bytes() || alignment == 0,
                  ...
                  "The intent is for the parameter is only as a workaround if either one of the "
                  "pointers is not properly aligned. "
                  "If you use it to do more bytes per copy than ggml_cuda_max_cpy_bytes() the "
                  "reads and writes may not be coalesced. "
...
static __device__ __forceinline__ float ggml_cuda_ue4m3_to_fp32(uint8_t x) {
    ...
    const uint32_t bits =
        x * (x != 0x7F && x != 0xFF);  // Convert NaN to 0.0f to match CPU implementation.
...
#endif  // defined(GGML_USE_HIP) && defined(CDNA3) && defined(FP8_AVAILABLE) && HIP_VERSION >=
        // 60200000
}
...
static __device__ __forceinline__ float get_alibi_slope(const float max_bias,
                                                        const uint32_t h,
...
struct ggml_cuda_pool_alloc {
    ...
    ggml_cuda_pool_alloc(ggml_cuda_pool& pool, size_t size) : pool(&pool) { alloc(size); }
...
struct ggml_tensor_extra_gpu {
    ...
    cudaEvent_t events[GGML_CUDA_MAX_DEVICES]
                      [GGML_CUDA_MAX_STREAMS];  // events for synchronizing multiple GPUs
};
...
struct ggml_cuda_graph {
    ...
    bool cublaslt_bias_autotune_warmup_complete = false;
    uint64_t uid = 0;
...
struct ggml_cuda_concurrent_event {
    ...
    // 2. check whether all srcs are either within the branch or outside the nodes covered by
    // ggml_cuda_concurrent_event we assume all nodes have the same buffer
    bool is_valid() const {
        ...
        for (const auto& [tensor, stream] : stream_mapping) {
            ...
            if ((t_start <= join_start && join_start < t_end) ||
                (join_start <= t_start && t_start < join_end)) {
                continue;
            }
        ...
        for (const auto& [tensor, stream] : stream_mapping) {
            ...
            for (int i = 0; i < n_streams; ++i) {
                ...
                auto it =
                    std::lower_bound(write_ranges[i].begin(), write_ranges[i].end(), data_range);

                if (it != write_ranges[i].end()) {
                    const std::pair<int64_t, int64_t>& other = *it;

                    // std::lower_bound returns the first element where other >= data_range
                    // (lexicographically). This guarantees other.first >= data_range.first.
...
struct ggml_cuda_stream_context {
    ...
    void reset() { concurrent_events.clear(); }
};
...
struct ggml_backend_cuda_context {
    ...
    cublasLtHandle_t cublaslt_handle() { return cublaslt_handle(device); }
#endif
