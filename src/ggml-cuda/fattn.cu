...
static __global__ void fattn_pad_head56_to64_f16(const float* __restrict__ src,
...
static __global__ void fattn_pad_head_f32_q_kv_f16_same_shape(const float* __restrict__ Q,
...
static __global__ void fattn_pad_head56_to64_q_f32_kv_f16(const float* __restrict__ Q,
                                                          const float* __restrict__ K,
                                                          const float* __restrict__ V,
                                                          float* __restrict__ Q_dst,
                                                          half* __restrict__ K_dst,
...
static __global__ void fattn_pad_head56_to64_q_f32_kv_f16_row4(const float* __restrict__ Q,
                                                               const float* __restrict__ K,
                                                               const float* __restrict__ V,
                                                               float* __restrict__ Q_dst,
                                                               half* __restrict__ K_dst,
...
static __global__ void fattn_pad_head56_to64_qk_f32_f16_v56_f16(const float* __restrict__ Q,
                                                                const float* __restrict__ K,
                                                                const float* __restrict__ V,
                                                                float* __restrict__ Q_dst,
                                                                half* __restrict__ K_dst,
...
static __global__ void fattn_pad_head56_to64_q_f32_kv_f16_mixed(const float* __restrict__ Q,
                                                                const float* __restrict__ K,
                                                                const float* __restrict__ V,
                                                                float* __restrict__ Q_dst,
                                                                half* __restrict__ K_dst,
...
static __global__ void fattn_pad_head56_to64_q_f32_kv_f16_mixed_row4(const float* __restrict__ Q,
                                                                     const float* __restrict__ K,
                                                                     const float* __restrict__ V,
                                                                     float* __restrict__ Q_dst,
                                                                     half* __restrict__ K_dst,
    ...
    if (row < q_rows) {
        ...
        if (lane == 14) {
            ...
            *reinterpret_cast<float4*>(Q_dst + dst_base + 60) = zero4;
            ...
            const int64_t q_base = i1 * q_s1 + i2 * q_s2 + i3 * q_s3 + d;
            const float4 q4 =
                make_float4(Q[q_base + 0], Q[q_base + 1], Q[q_base + 2], Q[q_base + 3]);
            *reinterpret_cast<float4*>(Q_dst + dst_base + d) = q4;
        }
    }
    ...
    if (row < k_rows) {
        ...
        if (lane == 14) {
            ...
            *reinterpret_cast<half2*>(V_dst + dst_base + 62) = make_half2(0.0f, 0.0f);
            ...
            const int64_t v_base = i1 * v_s1 + i2 * v_s2 + i3 * v_s3 + d;
            const float4 k4 =
                make_float4(K[k_base + 0], K[k_base + 1], K[k_base + 2], K[k_base + 3]);
            const float4 v4 =
                make_float4(V[v_base + 0], V[v_base + 1], V[v_base + 2], V[v_base + 3]);
            *reinterpret_cast<half2*>(K_dst + dst_base + d + 0) = __floats2half2_rn(k4.x, k4.y);
            *reinterpret_cast<half2*>(K_dst + dst_base + d + 2) = __floats2half2_rn(k4.z, k4.w);
            *reinterpret_cast<half2*>(V_dst + dst_base + d + 0) = __floats2half2_rn(v4.x, v4.y);
            *reinterpret_cast<half2*>(V_dst + dst_base + d + 2) = __floats2half2_rn(v4.z, v4.w);
...
static __global__ void fattn_pad_head56_to64_qk_f32_f16_v56_f16_mixed(const float* __restrict__ Q,
                                                                      const float* __restrict__ K,
                                                                      const float* __restrict__ V,
                                                                      float* __restrict__ Q_dst,
                                                                      half* __restrict__ K_dst,
...
static __global__ void fattn_pad_head56_to64_q_f32_kv_f16_contiguous(const float* __restrict__ Q,
                                                                     const float* __restrict__ K,
                                                                     const float* __restrict__ V,
                                                                     float* __restrict__ Q_dst,
                                                                     half* __restrict__ K_dst,
...
static __global__ void fattn_copy_head56_to_f16(const float* __restrict__ src,
                                                half* __restrict__ dst,
                                                const int64_t ne1,
                                                const int64_t ne2,
                                                const int64_t ne3,
...
static __global__ void fattn_slice_head64_to56_f32(const float* __restrict__ src,
                                                   float* __restrict__ dst,
                                                   const int64_t ne1,
                                                   const int64_t ne2,
                                                   const int64_t ne3,
...
static __global__ void fattn_slice_head64_to56_f32_contiguous(const float* __restrict__ src,
                                                              float* __restrict__ dst,
                                                              const int64_t n) {
    const int64_t i = int64_t(blockIdx.x) * blockDim.x + threadIdx.x;
    if (i >= n) {
...
template <int D, int DP>
static __global__ void fattn_slice_head_padded_to_f32(const float* __restrict__ src,
                                                      float* __restrict__ dst,
                                                      const int64_t ne1,
                                                      const int64_t ne2,
                                                      const int64_t ne3,
...
static void ggml_cuda_flash_attn_ext_head56_pad_mma_f16(ggml_backend_cuda_context& ctx,
                                                        ggml_tensor* dst);
...
template <int DKQ,
          ...
          int DV_DST = DV,
          int D_SRC = DKQ,
          bool no_mask_no_sinks = false>
static void ggml_cuda_flash_attn_ext_mma_f16_switch_ncols1(ggml_backend_cuda_context& ctx,
                                                           ggml_tensor* dst) {
    const int cc = ggml_cuda_info().devices[ggml_cuda_get_device()].cc;
    const ggml_tensor* Q = dst->src[0];

    if constexpr (DKQ == 32 && DV == 32 && ncols2 == 1) {
        const char* forced = getenv("GGML_CUDA_FATTN32_MMA_NCOLS1");
        const char* match = getenv("GGML_CUDA_FATTN32_MMA_NCOLS1_MATCH");
        if (forced != nullptr &&
            (match == nullptr || ggml_cuda_name_matches_csv(dst->name, match))) {
            if (strcmp(forced, "8") == 0) {
                ggml_cuda_flash_attn_ext_mma_f16_case<DKQ,
                                                      DV,
                                                      8,
                                                      ncols2,
                                                      DV_DST,
                                                      D_SRC,
                                                      no_mask_no_sinks>(ctx, dst);
                return;
            }
    ...
    if constexpr (DKQ == 64 && DV == 64 && ncols2 == 1) {
        const char* forced = getenv("GGML_CUDA_FATTN64_MMA_NCOLS1");
        const char* match = getenv("GGML_CUDA_FATTN64_MMA_NCOLS1_MATCH");
        if (forced != nullptr &&
            (match == nullptr || ggml_cuda_name_matches_csv(dst->name, match))) {
            if (strcmp(forced, "8") == 0) {
                ggml_cuda_flash_attn_ext_mma_f16_case<DKQ,
    ...
    if (ggml_cuda_highest_compiled_arch(cc) == GGML_CUDA_CC_TURING || amd_wmma_available(cc) ||
        Q->ne[1] <= 32 / ncols2) {
        ggml_cuda_flash_attn_ext_mma_f16_case<DKQ,
                                              DV,
                                              32 / ncols2,
...
static void ggml_cuda_flash_attn_ext_mma_f16_switch_ncols2(ggml_backend_cuda_context& ctx,
                                                           ggml_tensor* dst) {
    const int cc = ggml_cuda_info().devices[ggml_cuda_get_device()].cc;
    ...
    // Edge cases like no mask, ALiBi, unpadded K/V, or misaligned addresses for large data
    // transfers
    ...
    if (cc == GGML_CUDA_CC_VOLTA) {
        if (use_gqa_opt && gqa_ratio % 8 == 0) {
            ggml_cuda_flash_attn_ext_mma_f16_switch_ncols1<DKQ,
                                                           DV,
                                                           8,
                                                           DV_DST,
                                                           D_SRC,
    ...
    if (use_gqa_opt && gqa_ratio > 4) {
        ggml_cuda_flash_attn_ext_mma_f16_switch_ncols1<DKQ, DV, 8, DV_DST, D_SRC, no_mask_no_sinks>(
            ctx, dst);
        return;
    }

    if (use_gqa_opt && gqa_ratio > 2) {
        ggml_cuda_flash_attn_ext_mma_f16_switch_ncols1<DKQ, DV, 4, DV_DST, D_SRC, no_mask_no_sinks>(
            ctx, dst);
        return;
    }
    ...
    if constexpr (DKQ <= 256) {
        ...
        ggml_cuda_flash_attn_ext_mma_f16_switch_ncols1<DKQ, DV, 1, DV_DST, D_SRC, no_mask_no_sinks>(
            ctx, dst);
    } else {
        GGML_ABORT("fatal error");
    }
...
static void ggml_cuda_flash_attn_ext_mma_f16(ggml_backend_cuda_context& ctx, ggml_tensor* dst) {
    ...
    switch (Q->ne[0]) {
        ...
        case 64:
            ...
            if (mask == nullptr && KQV->src[4] == nullptr &&
                getenv("GGML_CUDA_DISABLE_FATTN64_NOMASK_FAST") == nullptr) {
                ggml_cuda_flash_attn_ext_mma_f16_switch_ncols2<64, 64, 64, 64, true>(ctx, dst);
            } else {
        ...
        case 256:
            ...
            if (mask == nullptr && KQV->src[4] == nullptr &&
                getenv("GGML_CUDA_DISABLE_FATTN256_NOMASK_FAST") == nullptr) {
        ...
        case 512:
            ...
            break;
        case 576: {
            // For Deepseek, go straight to the ncols1 switch to avoid compiling unnecessary
            // kernels.
            GGML_ASSERT(V->ne[0] == 512);
            float max_bias = 0.0f;
            memcpy(&max_bias, (const float*) KQV->op_params + 1, sizeof(float));
...
static void ggml_cuda_flash_attn_ext_head72_pad_mma_f16(ggml_backend_cuda_context& ctx,
    ...
    const bool direct_head72_output =
        getenv("GGML_CUDA_DISABLE_FATTN72_DIRECT_OUT") == nullptr && ggml_is_contiguous(dst);
    ...
    if (direct_head72_output && getenv("GGML_CUDA_DISABLE_FATTN72_INLINE_PACK") == nullptr) {
        ...
        if (profile_fattn72) {
            ...
            fprintf(stderr,
                    "GGML_CUDA_PROFILE_FATTN72 pack_ms=0.000000 mma_ms=%.6f slice_ms=0.000000 "
                    "total_ms=%.6f "
    ...
    fattn_pad_head_f32_q_kv_f16_same_shape<72, 80>
        <<<grid_size, block_size, 0, stream>>>((const float*) Q->data,
                                               (const float*) K->data,
                                               (const float*) V->data,
                                               Q_pad.ptr,
                                               K_pad.ptr,
    ...
    if (!direct_head72_output) {
        ...
        fattn_slice_head_padded_to_f32<72, 80>
            <<<slice_grid_size, block_size, 0, stream>>>(dst_pad.ptr,
                                                         (float*) dst->data,
                                                         dst->ne[1],
                                                         dst->ne[2],
                                                         dst->ne[3],
...
static void ggml_cuda_flash_attn_ext_head56_pad_mma_f16(ggml_backend_cuda_context& ctx,
    ...
    if (direct_head56_output && inline_head56_f32_compatible && !native_v56_mma &&
        ...
        if (profile_fattn56) {
            ...
            CUDA_CHECK(cudaEventElapsedTime(&mma_ms, profile_start, profile_after_mma));
            fprintf(stderr,
                    "GGML_CUDA_PROFILE_FATTN56 pack_ms=0.000000 mma_ms=%.6f slice_ms=0.000000 "
                    "total_ms=%.6f "
    ...
    ggml_cuda_pool_alloc<half> V_pad(pool,
                                     (native_v56_mma ? 56 : 64) * V->ne[1] * V->ne[2] * V->ne[3]);
    ggml_cuda_pool_alloc<float> dst_pad(pool);
    ...
    auto launch_pad_f32 = [&](const ggml_tensor* src, float* tmp) {
        ...
        const int grid_size = (n + block_size - 1) / block_size;
        fattn_pad_head56_to64_f32<<<grid_size, block_size, 0, stream>>>((const float*) src->data,
                                                                        tmp,
                                                                        src->ne[1],
                                                                        src->ne[2],
                                                                        src->ne[3],
    ...
    auto launch_pad_f16 = [&](const ggml_tensor* src, half* tmp) {
        const int64_t n = 64 * src->ne[1] * src->ne[2] * src->ne[3];
        const int grid_size = (n + block_size - 1) / block_size;
        fattn_pad_head56_to64_f16<<<grid_size, block_size, 0, stream>>>((const float*) src->data,
                                                                        tmp,
                                                                        src->ne[1],
                                                                        src->ne[2],
                                                                        src->ne[3],
    ...
    auto launch_copy_v56_f16 = [&](const ggml_tensor* src, half* tmp) {
        ...
        const int grid_size = (n + block_size - 1) / block_size;
        fattn_copy_head56_to_f16<<<grid_size, block_size, 0, stream>>>((const float*) src->data,
                                                                       tmp,
                                                                       src->ne[1],
                                                                       src->ne[2],
                                                                       src->ne[3],
    ...
    if (same_qkv_shape && getenv("GGML_CUDA_DISABLE_FATTN56_COMBINED_PACK") == nullptr) {
        ...
        const bool can_use_mixed_pack = getenv("GGML_CUDA_DISABLE_FATTN56_MIXED_PACK") == nullptr &&
                                        K->ne[1] == V->ne[1] && K->ne[2] == V->ne[2] &&
                                        K->ne[3] == V->ne[3] && Q->ne[2] == K->ne[2] &&
                                        Q->ne[3] == K->ne[3];
        if (can_use_mixed_pack) {
            ...
            const int grid_size = (n + block_size - 1) / block_size;
            if (native_v56_mma) {
                fattn_pad_head56_to64_qk_f32_f16_v56_f16_mixed<<<grid_size,
                                                                 block_size,
                                                                 0,
                                                                 stream>>>(
                    (const float*) Q->data,
                ...
                if (getenv("GGML_CUDA_DISABLE_FATTN56_MIXED_ROW4_PACK") == nullptr) {
                    ...
                    const int row4_grid_size = (rows + rows_per_block - 1) / rows_per_block;
                    fattn_pad_head56_to64_q_f32_kv_f16_mixed_row4<<<row4_grid_size,
                                                                    row4_block_size,
                                                                    0,
                                                                    stream>>>(
                        (const float*) Q->data,
    ...
    fattn_make_contiguous_f32_head64_tensor(
        dst64, direct_head56_output ? (float*) dst->data : dst_pad.ptr);
    dst64.src[0] = &Q64;
    dst64.src[1] = &K64;
    dst64.src[2] = &V64;
    dst64.src[3] = nullptr;
    ...
    if (!direct_head56_output) {
        ...
        const int grid_size = (n + block_size - 1) / block_size;
        if (getenv("GGML_CUDA_DISABLE_FATTN56_CONTIGUOUS_PACK") == nullptr &&
            ggml_is_contiguous(dst)) {
...
#define FATTN_VEC_CASE(D, type_K, type_V)                                                   \
    {                                                                                       \
        const bool type_K_okay =                                                            \
            K->type == (type_K) || (K->type == GGML_TYPE_F32 && (type_K) == GGML_TYPE_F16); \
        const bool type_V_okay =                                                            \
            V->type == (type_V) || (V->type == GGML_TYPE_F32 && (type_V) == GGML_TYPE_F16); \
...
static bool ggml_cuda_str_contains_n(const char* haystack,
                                     const char* needle,
...
static bool ggml_cuda_name_matches_csv(const char* name, const char* csv) {
    ...
    while (*token != '\0') {
        ...
        const size_t len = end == nullptr ? strlen(token) : (size_t) (end - token);
        if (ggml_cuda_str_contains_n(name, token, len)) {
            return true;
        }
        if (end == nullptr) {
            return false;
...
static bool ggml_cuda_should_use_fattn32_mma(const ggml_tensor* dst, const int cc) {
    if (!turing_mma_available(cc)) {
        return false;
    }

    const char* mode = getenv("GGML_CUDA_ENABLE_FATTN32_MMA");
    if (mode == nullptr) {
        return strstr(dst->name, "sam31_mem_attn_layer") != nullptr &&
               (strstr(dst->name, "_sa_fattn") != nullptr ||
                strstr(dst->name, "_ca_fattn") != nullptr);
    }
    if (mode[0] == '0') {
        return false;
    }
    if (mode[0] == '\0' || strcmp(mode, "1") == 0 || strcmp(mode, "all") == 0) {
        return true;
    }
    if (strncmp(mode, "match:", 6) == 0) {
        return strstr(dst->name, mode + 6) != nullptr;
    }
    if (strncmp(mode, "sam31-ca+match:", 15) == 0) {
        return strstr(dst->name, "_ca_fattn") != nullptr || strstr(dst->name, mode + 15) != nullptr;
    }
    if (strncmp(mode, "sam31-ca+matches:", 17) == 0) {
        return strstr(dst->name, "_ca_fattn") != nullptr ||
               ggml_cuda_name_matches_csv(dst->name, mode + 17);
    }
    if (strcmp(mode, "sam31-sa") == 0) {
        return strstr(dst->name, "_sa_fattn") != nullptr;
    }
    if (strcmp(mode, "sam31-ca") == 0) {
        return strstr(dst->name, "_ca_fattn") != nullptr;
    }
    return false;
}

static bool ggml_cuda_can_use_fattn_mma_f16_for_head_dim(const ggml_tensor* Q, const int cc) {
    if (!(turing_mma_available(cc) || volta_mma_available(cc) ||
          (amd_wmma_available(cc) && GGML_CUDA_CC_IS_RDNA4(cc)) || amd_mfma_available(cc))) {
        return false;
    }
    switch (Q->ne[0]) {
        case 32:
...
static bool ggml_cuda_should_use_sam3_vit_fattn_mma(const ggml_tensor* dst,
                                                    const ggml_tensor* Q,
                                                    const int cc) {
    if (getenv("GGML_CUDA_DISABLE_SAM3_VIT_FATTN_MMA") != nullptr) {
        return false;
    }
    if (Q->ne[0] != 64 || !ggml_cuda_can_use_fattn_mma_f16_for_head_dim(Q, cc)) {
        return false;
    }
    return strstr(dst->name, "sam3_vit_block") != nullptr;
}
...
static best_fattn_kernel ggml_cuda_get_best_fattn_kernel(const int device, const ggml_tensor* dst) {
    ...
    // The kernel versions without this optimization are also used for ALiBi, if there is no mask,
    // or if the KV cache is not padded,
    bool gqa_opt_applies =
        gqa_ratio >= 2 && mask && max_bias == 0.0f && K->ne[1] % FATTN_KQ_STRIDE == 0;
    for (const ggml_tensor* t : {Q, K, V, mask}) {
        if (t == nullptr || ggml_is_quantized(t->type)) {
            continue;
        }
    ...
    // For small batch sizes the vector kernel may be preferable over the kernels optimized for
    // large batch sizes:
    const bool can_use_vector_kernel =
        Q->ne[0] <= 256 && Q->ne[0] % 64 == 0 && K->ne[1] % FATTN_KQ_STRIDE == 0;

    if (ggml_cuda_env_enabled("GGML_CUDA_FORCE_FATTN_TILE")) {
        return BEST_FATTN_KERNEL_TILE;
    }
    if (const char* match = getenv("GGML_CUDA_FORCE_FATTN_TILE_MATCH")) {
        if (ggml_cuda_name_matches_csv(dst->name, match)) {
            return BEST_FATTN_KERNEL_TILE;
        }
    }
    if (ggml_cuda_env_enabled("GGML_CUDA_FORCE_FATTN_VEC")) {
        return can_use_vector_kernel ? BEST_FATTN_KERNEL_VEC : BEST_FATTN_KERNEL_TILE;
    }
    if (const char* match = getenv("GGML_CUDA_FORCE_FATTN_VEC_MATCH")) {
        if (ggml_cuda_name_matches_csv(dst->name, match)) {
            return can_use_vector_kernel ? BEST_FATTN_KERNEL_VEC : BEST_FATTN_KERNEL_TILE;
        }
    }
    if (getenv("GGML_CUDA_DISABLE_SAM31_MEM_ATTN_L0_SA_TILE") == nullptr &&
        ggml_cuda_name_matches_csv(dst->name, "sam31_mem_attn_layer0_sa_fattn")) {
        return BEST_FATTN_KERNEL_TILE;
    }
    if (ggml_cuda_env_enabled("GGML_CUDA_FORCE_FATTN_MMA")) {
        if (ggml_cuda_can_use_fattn_mma_f16_for_head_dim(Q, cc)) {
            return BEST_FATTN_KERNEL_MMA_F16;
        }
    }
    if (const char* match = getenv("GGML_CUDA_FORCE_FATTN_MMA_MATCH")) {
        if (ggml_cuda_name_matches_csv(dst->name, match) &&
            ggml_cuda_can_use_fattn_mma_f16_for_head_dim(Q, cc)) {
            return BEST_FATTN_KERNEL_MMA_F16;
        }
    }

    if (ggml_cuda_should_use_sam3_vit_fattn_mma(dst, Q, cc)) {
        return BEST_FATTN_KERNEL_MMA_F16;
    }

    if (Q->ne[0] == 32 && ggml_cuda_should_use_fattn32_mma(dst, cc)) {
        return BEST_FATTN_KERNEL_MMA_F16;
    }
    ...
    if (volta_mma_available(cc) && Q->ne[0] != 40 && Q->ne[0] != 56 && Q->ne[0] != 72) {
        ...
        if (Q->ne[1] * gqa_ratio_eff <= 16) {
            return BEST_FATTN_KERNEL_TILE;  // On Volta tensor cores are only faster for
    ...
    if (ggml_cuda_should_use_wmma_fattn(cc) && K->ne[1] % FATTN_KQ_STRIDE == 0 && Q->ne[0] != 40 &&
        Q->ne[0] != 56 && Q->ne[0] != 72 && Q->ne[0] != 512 && Q->ne[0] != 576) {
    ...
    if (amd_wmma_available(cc) && GGML_CUDA_CC_IS_RDNA4(cc) && gqa_opt_applies && Q->ne[0] <= 128 &&
        Q->ne[0] != 40 && Q->ne[0] != 56 && Q->ne[0] != 72) {
        if (can_use_vector_kernel) {
            if (!ggml_is_quantized(K->type) && !ggml_is_quantized(V->type)) {
                if (Q->ne[1] == 1) {
                    if (!gqa_opt_applies) {
        ...
        if (Q->ne[1] * gqa_ratio_eff <= 8) {
            return BEST_FATTN_KERNEL_TILE;  // AMD WMMA is only faster if the full tile width of 16
                                            // can be utilized.
        }
        return BEST_FATTN_KERNEL_MMA_F16;
    }
    ...
    if (amd_mfma_available(cc) && Q->ne[0] != 40 && Q->ne[0] != 56 && Q->ne[0] != 72 &&
        Q->ne[0] != 256 && Q->ne[0] != 512 && Q->ne[0] != 576) {
...
void ggml_cuda_flash_attn_ext(ggml_backend_cuda_context& ctx, ggml_tensor* dst) {
...
#ifdef GGML_CUDA_USE_CUDNN_SDPA
    if (ggml_cuda_cudnn_sdpa_head64(ctx, dst)) {
        return;
    }
    if (ggml_cuda_cudnn_sdpa_head32(ctx, dst)) {
        return;
    }
#endif
    if (Q->ne[0] == 56 && K->ne[0] == 56 && V->ne[0] == 56 && mask == nullptr && sinks == nullptr &&
    ...
    if (Q->ne[0] == 72 && K->ne[0] == 72 && V->ne[0] == 72 && Q->ne[1] == K->ne[1] &&
        Q->ne[1] == V->ne[1] && Q->ne[2] == K->ne[2] && Q->ne[2] == V->ne[2] &&
        Q->ne[3] == K->ne[3] && Q->ne[3] == V->ne[3] && mask == nullptr && sinks == nullptr &&
        Q->type == GGML_TYPE_F32 && K->type == GGML_TYPE_F32 && V->type == GGML_TYPE_F32 &&
        turing_mma_available(ggml_cuda_info().devices[ctx.device].cc) &&
    ...
    switch (ggml_cuda_get_best_fattn_kernel(ggml_cuda_get_device(), dst)) {
        case BEST_FATTN_KERNEL_NONE:
            ...
            fprintf(stderr,
                    ...
                    "K[%lld,%lld,%lld,%lld] %s V[%lld,%lld,%lld,%lld] %s mask=%s cc=%d highest=%d "
                    "turing=%d\n",
                    (long long) dst->src[0]->ne[0],
                    (long long) dst->src[0]->ne[1],
                    (long long) dst->src[0]->ne[2],
                    (long long) dst->src[0]->ne[3],
                    ggml_type_name(dst->src[0]->type),
                    (long long) dst->src[1]->ne[0],
                    (long long) dst->src[1]->ne[1],
                    (long long) dst->src[1]->ne[2],
                    (long long) dst->src[1]->ne[3],
                    ggml_type_name(dst->src[1]->type),
                    (long long) dst->src[2]->ne[0],
                    (long long) dst->src[2]->ne[1],
                    (long long) dst->src[2]->ne[2],
                    (long long) dst->src[2]->ne[3],
